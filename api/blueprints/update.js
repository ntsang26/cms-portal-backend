...
module.exports = function updateOneRecord(req, res) {
    ...
    var criteria = {};
    criteria['sid'] = queryOptions.criteria.where[Model.primaryKey];
    ...
    Model.findOne(
        ...
        .exec(async function (err, matchingRecord) {
            if (err) {
                switch (err.name) {
            ...
            // TuPT add 02/04/2020
            queryOptions.valuesToSet.sid = queryOptions.valuesToSet[Model.primaryKey]
            delete queryOptions.valuesToSet[Model.primaryKey]
            if (Model.validateParamUpdate) {
                let validate = await Model.validateParamUpdate(queryOptions.valuesToSet, req.user.company)
                if (validate) return res.json(validate)
            }
            if (Model.attributes.updatedBy) {
                queryOptions.valuesToSet.updatedBy = { sid: req.user.sid, name: req.user.name, role: req.user.role }
            }
            // end
