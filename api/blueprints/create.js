...
module.exports = function createRecord(req, res) {
    ...
    async.reduce(_.keys(Model.attributes), [], function (memo, attrName, nextAttrName) {
        ...
        // TuPT add 02/04/2020
        if (Model.validateParamCreate) {
            let validate = await Model.validateParamCreate(data, req.user.company)
            if (validate) return res.json(validate)
        }
        if (Model.attributes.createdBy) {
            data.createdBy = { sid: req.user.sid, name: req.user.name, role: req.user.role }
        }
        ...
        //addBy Tuyennt
        data.sid = sails.services.modelhelper.generateDigitCode(16)
